...
router.post('/visual-search', async (req, res) => {
  try {
    ...
    const resultsPerItem = await Promise.all(items.map(async (itemData, index) => {
      ...
      const hasStyleRerank = styleContrastVector.length > 0 && !!shoeStyle;
      const PASTEL_COLORS = new Set(['lavender', 'purple', 'pink']);
      console.log(`🎨 Item ${index} → detectedColor: "${detectedColor}", hasColor: ${hasColor}, patternRerank: ${isPattern}, shoeStyle: "${shoeStyle || 'none'}"`);
      ...
      if (hasColorRerank && products.length > 0) {
        ...
        if (products.length === 0 && vectorCandidates.length > 0) {
          console.warn(`⚠️ Item ${index}: colour pipeline returned 0 — falling back to vector order`);
      ...
      if (hasStyleRerank && products.some(p => p.colorScore != null)) {
        ...
        console.log(`👡 Item ${index}: shoe style-priority reorder applied (${shoeStyle})`);
